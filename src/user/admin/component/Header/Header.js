...
const Header = (props) => {
    ...
    // const cartNumber = useSelector(state => state.cartinToolkit)

    // const cartCount = cartNumber.carting.quantity ? cartNumber.quantity.reduce((total, item) => total + item, 0) : 0;

    const cart = useSelector(state => state.cartinToolkit);

    const cartCount = cart.carting.reduce((total, item) => total + item.quantity, 0);
